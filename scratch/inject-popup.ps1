...
foreach ($file in $files) {
    ...
    # Insert the popup just before the first <script> tag
    $content = $content -replace '(<script>)', ($newPopup + "`n`$1"), 1
    
    # Fix toggle function if still old style
...
"@
    ...
    Write-Host "Injected popup into: $file"
}

# Final verify
$check1 = Select-String -Path 'd:\creashiiftads\public\*.html' -Pattern 'Mobile Navigation Popup' -List | Select-Object -ExpandProperty Filename
$check2 = Select-String -Path 'd:\creashiiftads\public\*.html' -Pattern 'translate-x-full' -List | Select-Object -ExpandProperty Filename
Write-Host "`nFiles with new popup: $($check1.Count)"
Write-Host "Files with old drawer: $($check2.Count)"
